...
const NavigationMenu = () => {
  ...
  useEffect(() => {
    fetchGlobalData('NavMenu').then(({ data, message }: ResponseDataWithLoadingAndError<IMenuItem>) => {
      setMenuData({
    ...
    <Navbar>
      ...
      <NavbarContent className="hidden sm:flex gap-4" justify="center">
        {menuData.data.map((menuItem: IMenuItem, idx: number) => (
          <MenuItem key={idx} title={menuItem.title} alt={menuItem.alt} link={menuItem.link} />
        ))}
      </NavbarContent>
    </Navbar>
  )
