...
export default function ProductCard({ product, cssClasses }: Props) {
  const { productId, productName, productDescription, price, quantity, isProductAvailable, productImages, categories } = product;
  return (
    <Card isFooterBlurred className={`w-full h-[300px] ${cssClasses}`} isPressable shadow='sm' onPress={() => console.log('pressed')} data-product-id={productId}>
      <CardHeader className="absolute z-10 top-1 flex-col items-start">
        ...
        <span>{quantity} - {(isProductAvailable) ? "product available" : "product not available "}</span>
      </CardHeader>
      <Image
        removeWrapper
